...
class case_execer:
    ...
    def exec_case(self):
        ...
        try:
            ...
            self.log.logMsg(4, f'case execute stop casue:{e}')
        finally:
            self.ec.runreslut_count()  # 统计的功能 总计，成功，失败

    def exec_case_step(self, step:CaseStep):
        ...
        if step.Enabled == 1:  # 执行操作
            if step.Behavior != "check":  # 如果在excel的操作选项中不包含 check
                ...
                self.total, self.success, self.fail = self.ec.casesResult(self.bm, step, self.fileobj, returnvalue,case_execer.filepathname)
